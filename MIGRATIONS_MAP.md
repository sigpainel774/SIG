...
| 96 | `20260816100000_session_timeout_rules.sql` | 2026-08-16 | Tabela de regras de encerramento compulsório de sessão por horário (desativada por padrão) com granularidade de rede, secretaria, escola e nível (exclusivo Nível >= 2). RPC `get_session_timeout_rules_for_user`. | `public.session_timeout_rules` | `SECURITY DEFINER` + RLS Superadmin |
| 97 | `20260817000000_gestao_flexivel_atividades_e_alertas.sql` | 2026-08-17 | Adiciona colunas `pontos_maximos` e `enviado_impressao` em `atividades_secretaria`, cria tabela `notas_atividades` para até 10 atividades por aluno/trimestre e cria a RPC `verificar_pendencias_pontuacao_trimestre` com alerta a 5 dias do prazo para Professor e Diretor. | `atividades_secretaria`, `notas_atividades`, `notifications`, `prazos_unidades` | `SECURITY DEFINER` + RLS |
| 98 | `20260817010000_calendario_academico_schema.sql` | 2026-08-17 | Criação do schema do Calendário Acadêmico da Rede Municipal de Educação (`calendarios_academicos`, `calendario_eventos`, `calendario_historico`), índices e políticas RLS. | `public.calendarios_academicos`, `public.calendario_eventos`, `public.calendario_historico` | `dev_all_authenticated` / RLS |
| 99 | `20260817013000_vincular_calendario_a_alertas_avaliacoes.sql` | 2026-08-17 | Atualização da RPC `verificar_pendencias_pontuacao_trimestre` integrando as datas de encerramento de trimestres oficiais de `calendarios_academicos` e blindagem da exceção do EMAEE. | `public.calendarios_academicos`, `public.notifications`, `public.atividades_secretaria` | `SECURITY DEFINER` + RLS |
