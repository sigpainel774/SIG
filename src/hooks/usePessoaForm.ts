...
export function usePessoaForm(options?: PessoaFormOptions) {
  ...
  const populatePessoais = (data: any) => {
    ...
    setCorRaca(data.cor_raca || data.corRacaAluno || data.corRaca || options?.corRacaDefault || 'Não declarado')
    ...
    const rawCidadeNasc = data.municipio_nascimento || data.cidadeNascAluno || data.naturalidadeAluno || data.cidadeNasc || data.naturalidade || data.cidade_nascimento || ''
    setCidadeNasc(rawCidadeNasc ? formatNameTitleCase(rawCidadeNasc) : '')
    const rawUfNasc = data.uf_nascimento || data.ufNascAluno || data.ufNasc || data.uf_naturalidade || options?.ufNascDefault || ''
    setUfNasc(rawUfNasc ? rawUfNasc.toUpperCase() : '')
