...
export function IndicadoresClient() {
  ...
  // ── Disparar Alertas Duplos de Frequência Pendente ──
  const handleTriggerFrequenciaAlerts = async (itemIndividual?: FrequenciaPendente) => {
    ...
    try {
      ...
      if (semUserAuth.length > 0) {
        toast.warning(`${semUserAuth.length} professor(es) não possuem login ativo de acesso: ${semUserAuth.join(', ')}`)
      }
    } catch (err: any) {
      console.error(err)
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="bg-[#121214] border border-[#232326] rounded-2xl p-5 shadow-xl space-y-3">
              ...
              <StandardTable
                ...
                emptyMessage="Nenhuma pendência de frequência encontrada."
              />
            </div>
          </div>
        </div>
