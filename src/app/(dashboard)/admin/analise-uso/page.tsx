...
export default function AnaliseUsoPage() {
  ...
  // 1. Carregar Sessões Ao Vivo (via API com modelo econômico user_presence)
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (data.active_sessions && isMounted.current) {
          setSessoesAtivas(data.active_sessions)
          return
        }
      }

      // Fallback via RPC
      const { data: rpcData, error } = await (supabase as any).rpc('get_active_presences_summary', { p_timeout_minutes: 3 })
