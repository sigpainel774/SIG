...
import { useAlunoSignaturePolling } from '../hooks/useAlunoSignaturePolling'
...
export function AlunoFormProvider({ children, props, isOpen, setIsOpen }: AlunoFormProviderProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (alunoEditar?.id) {
        ...
        if (error) throw error
        toast.success('Ficha do aluno updated com sucesso!')
      } else {
  ...
  const value: AlunoFormContextType = {
    alunoEditar,
    isEditMode,
    isFichaBloqueada,
    loading,
...
export function useAlunoForm() {
  ...
  if (context === undefined) {
    throw new Error('useAlunoForm must be used within an AlunoFormProvider')
  }
  return context
}
