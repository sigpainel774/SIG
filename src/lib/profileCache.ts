    ...
    async (): Promise<PerfilCache | null> => {
      ...
      const [acessosRes, vinculosRes] = await Promise.all([
        supabaseAdmin
          ...
          .select('id, funcionario_id, escola_id, orgao_id, secretarias_ids, nivel, ativo, cargos_gerenciados, pode_mural, pode_turmas, pode_funcionarios, pode_matriculas, pode_alunos, pode_ocorrencias, pode_atestados, pode_rh_rede, created_at')
