  ...
  return unstable_cache(
    async (): Promise<PerfilCache | null> => {
      ...
      const { data: funcData, error: funcError } = await supabaseAdmin
        .from('funcionarios')
        .select('id, nome, email, auth_user_id, cpf, cargo, foto_url, foto_avatar_path, foto_visualizacao_path, foto_updated_at, assinatura_url, primeiro_acesso, status, is_superadmin, is_conta_especial, is_conta_eja, is_profissional_aee, permitir_mensagens_globais, modalidade_ensino, tipo_vinculo, endereco, telefone, data_nascimento, created_at')
        .ilike('email', email)
        .maybeSingle()
      ...
      const [acessosRes, vinculosRes] = await Promise.all([
        supabaseAdmin
          ...
          .select('id, funcionario_id, escola_id, orgao_id, secretarias_ids, nivel, ativo, cargos_gerenciados, pode_mural, pode_turmas, pode_funcionarios, pode_matriculas, pode_alunos, pode_ocorrencias, pode_atestados, pode_rh_rede, pode_eja, created_at')
      ...
      return {
        funcionario: funcData as unknown as Funcionario,
