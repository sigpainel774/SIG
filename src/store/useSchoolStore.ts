...
export interface Escola {
  ...
  eja_ativo?: boolean
  notificar_pendencias_emaee?: boolean | null
        ...
        if (found) {
          ...
          try {
            ...
            const { data } = await supabase
              .from('escolas')
              .select('id, nome, logo_url, plano, modulos_ativos, endereco, telefone, inep, tipo, ativo, diretor_id, localizacao, latitude, longitude, assinatura_diretor_url, codigo, secretaria_id, is_teste, portal_pais_ativo, portal_comunicacoes_ativo, eja_ativo, notificar_pendencias_emaee, created_at, deleted_at, secretarias:secretaria_id(id, nome)')
              .eq('id', id)
              .is('deleted_at', null)
              .maybeSingle()
