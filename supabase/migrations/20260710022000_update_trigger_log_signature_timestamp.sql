-- Migration: Atualizar trigger de auditoria para monitorar alteração de assinatura com base em URL e timestamp
...
BEGIN
  ...
  old_resp_at := old_dm->>'assinatura_responsavel_at';
  new_resp_at := new_dm->>'assinatura_responsavel_at';

  old_func_url := old_dm->>'assinatura_funcionario_url';
  new_func_url := new_dm->>'assinatura_funcionario_url';
  old_func_at := old_dm->>'assinatura_funcionario_at';
  new_func_at := new_dm->>'assinatura_funcionario_at';
  ...
  IF (new_resp_url IS DISTINCT FROM old_resp_url OR new_resp_at IS DISTINCT FROM old_resp_at) THEN
    ...
    INSERT INTO public.audit_logs (
      ...
      NEW.id,
      jsonb_build_object('url', old_resp_url, 'student_name', student_name, 'timestamp', old_resp_at),
      jsonb_build_object('url', new_resp_url, 'student_name', student_name, 'user_agent', user_agent, 'timestamp', new_resp_at),
      client_ip
    );

    -- Limpar código temporário do responsável de forma segura pós-assinatura (evita erro de RLS de SELECT no frontend)
    IF TG_OP = 'UPDATE' AND NEW.codigo_temp_resp IS NOT NULL THEN
      UPDATE public.alunos
      SET codigo_temp_resp = NULL
      WHERE id = NEW.id;
    END IF;
  END IF;

  -- 2. Auditar alteração de Assinatura do Funcionário
  IF (new_func_url IS DISTINCT FROM old_func_url OR new_func_at IS DISTINCT FROM old_func_at) THEN
    ...
    INSERT INTO public.audit_logs (
      ...
      NEW.id,
      jsonb_build_object('url', old_func_url, 'student_name', student_name, 'timestamp', old_func_at),
      jsonb_build_object('url', new_func_url, 'student_name', student_name, 'user_agent', user_agent, 'timestamp', new_func_at),
      client_ip
    );

    -- Limpar código temporário do funcionário de forma segura pós-assinatura (evita erro de RLS de SELECT no frontend)
    IF TG_OP = 'UPDATE' AND NEW.codigo_temp_func IS NOT NULL THEN
      UPDATE public.alunos
      SET codigo_temp_func = NULL
      WHERE id = NEW.id;
    END IF;
  END IF;
