-- Migration: 20260822140000_fix_overloaded_rpcs_cleanup.sql
-- Data: 2026-08-22
-- Descrição: Limpeza de sobrecargas de funções obsoletas (get_relatorio_servidores, get_birthdays_of_month, get_user_navigation_trail_admin) para eliminar o erro 42725 (function is not unique) e restabelecer o Relatório de Servidores, KPIs da Home, Mural e Auditoria.

-- 1. Remoção da versão legada de 4 parâmetros de get_relatorio_servidores
DROP FUNCTION IF EXISTS public.get_relatorio_servidores(uuid, text, text, text);

-- 2. Remoção da versão legada de 1 parâmetro de get_birthdays_of_month
DROP FUNCTION IF EXISTS public.get_birthdays_of_month(integer);

-- 3. Remoção da versão legada de 2 parâmetros de get_user_navigation_trail_admin
DROP FUNCTION IF EXISTS public.get_user_navigation_trail_admin(uuid, integer);

-- 4. Garantir a definição oficial mais recente e segura de get_relatorio_servidores (5 parâmetros com p_status)
